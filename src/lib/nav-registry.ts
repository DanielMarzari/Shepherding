...
export const PAGE_REGISTRY: Record<string, PageDef> = {
  ...
  "audit-pushpay": { href: "/audit/pushpay", defaultLabel: "PushPay connections", activeAliases: ["PushPay connections"], description: "Every giver in the imported PushPay Transactions export and which person their gifts belong to — place by hand the ones no rule could, whether their details fitted several people (same name, or a shared household inbox) or nobody at all." },
  ...
  // The reports are builder pages (mir-*), gathered in their own hub layer.
  // This card opens that layer; /mir, the first version, redirects there too.
  mir: { href: "/more?layer=ministry-impact-reports", defaultLabel: "Ministry Impact Reports", activeAliases: ["Ministry Impact Reports"], description: "One page per ministry: the 2025 Logic Model as published — Resources, Activities, Outputs, Outcomes, Impact — with Outputs measured against live data where it exists." },
...
export const DEFAULT_NAV_CONFIG: NavConfig = {
  ...
  groups: [
    ...
    // hardcoded section list.
    {
      ...
      items: [P("pco"), P("pushpay"), P("constant-contact"), P("spotify"), P("credentials")],
    },
    {
