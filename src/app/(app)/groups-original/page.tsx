...
import { GroupsExplorer } from "../groups/groups-explorer";
    ...
    <AppShell active="Groups (original)" breadcrumb="Groups (original design)">
      <div className="px-5 md:px-7 py-7 space-y-6">
        <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300 flex items-center justify-between flex-wrap gap-2">
          ...
          <a href="/groups" className="font-medium underline">View the new builder version →</a>
        </div>
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Groups <span className="text-muted text-base font-normal">· original</span></h1>
          {groups.length === 0 && (
        ...
        {groups.length > 0 && (
          <Suspense
            ...
            fallback={<AttendanceTrendSkeleton scope="groups" />}
          >
            <AsyncAttendanceTrendCard
              orgId={session.orgId}
              trendScope="groups"
