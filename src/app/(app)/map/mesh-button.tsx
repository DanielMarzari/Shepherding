...
import { meshStatusAction, startMeshAction } from "./actions";
...
export function MeshButton({
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const s = await meshStatusAction();
        if (cancelled) return;
        setStatus(s);
        if (s.running) {
    ...
    <div className="flex items-center gap-3 flex-wrap">
      ...
      {running && status && (
        <span className="text-xs text-muted tnum">
          {status.processed.toLocaleString()} routed · {status.remaining.toLocaleString()} left
        </span>
      )}
